...
def count(founded_classes,im0):
  ...
  for i, (k, v) in enumerate(founded_classes.items()):
    if(k=="car" or k=="bus" or k=="truck" or k=="motorcycle"):
        tot+=int(v)
        a=f"{k} = {v}"
  ...
  return tot, align_bottom, align_right
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                if(90-tot*1.5<20):
                  print(Fore.RED + "Red Timer: 20s")
                else:
                  print(Fore.RED + f"Red Timer: {90-tot*1.5}")
                if(tot*1.5>90):
                  print(Fore.GREEN + "Green Timer: 90s")
                else:
                  print(Fore.GREEN + f"Green Timer: {tot*1.5}s")
            ...
            print(Fore.WHITE + f'{s}Done. ({(1E3 * (t2 - t1)):.1f}ms) Inference, ({(1E3 * (t3 - t2)):.1f}ms) NMS')
